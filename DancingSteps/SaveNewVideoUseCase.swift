                    ...
                    onNext: { (styles) in
                        for style in styles {
                            if (style.name == styleId && self.videoSavedFlag == false) {
                                video = Video(id: title, title: title, datetime: Date(), videoDescription: "DESCRIPTION TEST", url: writePath.absoluteString, style: style) // TODO: Be careful with this, how to improve it ?
